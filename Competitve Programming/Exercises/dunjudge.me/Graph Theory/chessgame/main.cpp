...
int main(){
    ...
    bool die=(rook1Eat||rook2Eat);//Currenlty die
    for (int i=0;i<8;i++){
        int dx=directions[i][0],dy=directions[i][1];
        xk = pos[3][0]+dx, yk=pos[3][1]+dy;
        if (!inRange(xk,yk)){continue;}
        if(xk==pos[2][0]&&yk==pos[2][1]){continue;}//Got king there
        rook1Eat = rookEat(pos[0][0],pos[0][1],xk,yk)&&
                        !(xk==pos[0][0] && yk==pos[0][1]);
        rook2Eat = rookEat(pos[1][0],pos[1][1],xk,yk)&&
                        !(xk==pos[1][0] && yk==pos[1][1]);
