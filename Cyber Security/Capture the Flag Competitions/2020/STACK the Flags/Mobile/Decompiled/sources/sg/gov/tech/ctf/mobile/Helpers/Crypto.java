package sg.gov.tech.ctf.mobile.Helpers;
...
public class Crypto {
    ...
    public String hexToBin(String plainText) {
        String binary = BuildConfig.FLAVOR;
        int n = plainText.length();
        for (int i = 0; i < n; i++) {
            String binary4B = "0000" + Long.toBinaryString(Long.valueOf(Long.parseUnsignedLong(plainText.charAt(i) + BuildConfig.FLAVOR, 16)).longValue());
            binary = binary + binary4B.substring(binary4B.length() - 4);
        }
        return binary;
    }

    public String binToHex(String plainText) {
        String hexa = Long.toHexString(Long.parseUnsignedLong(plainText, 2));
    ...
    public String xor(String a2, String b2) {
        String a3 = hexToBin(a2);
        String b3 = hexToBin(b2);
        String ans = BuildConfig.FLAVOR;
        for (int i = 0; i < a3.length(); i++) {
            ans = ans + ((char) (((a3.charAt(i) - '0') ^ (b3.charAt(i) - '0')) + 48));
        }
        return binToHex(ans);
    }

    public String addBin(String a2, String b2) {
        String ans = "00000000" + Long.toHexString((Long.parseUnsignedLong(a2, 16) + Long.parseUnsignedLong(b2, 16)) % this.f2962c);
        return ans.substring(ans.length() - 8);
    }

    public String f(String plainText) {
        String[] a2 = new String[4];
        for (int i = 0; i < 8; i += 2) {
            a2[i / 2] = this.f2960a[i / 2][(int) Long.parseUnsignedLong(hexToBin(plainText.substring(i, i + 2)), 2)];
        }
        return addBin(xor(addBin(a2[0], a2[1]), a2[2]), a2[3]);
    }
    ...
    public void keyGenerate(String key) {
        ...
        while (true) {
            ...
            if (i < strArr.length) {
                strArr[i] = xor(strArr[i], key.substring(j, j + 8));
                j = (j + 8) % key.length();
                i++;
            } else {
                return;
    ...
    public String round(int time, String plainText) {
        String left = plainText.substring(0, 8);
        String right = plainText.substring(8, 16);
        String left2 = xor(left, this.f2961b[time]);
        String right2 = xor(f(left2), right);
        return right2 + left2;
    }
    ...
    public String encrypt(String plainText) {
        ...
        String right2 = xor(right, this.f2961b[16]);
        String left2 = xor(left, this.f2961b[17]);
        return left2 + right2;
    }
}
