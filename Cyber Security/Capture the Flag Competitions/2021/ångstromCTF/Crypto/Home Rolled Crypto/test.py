...
class DeCipher:
    ...
    def __init__(self, key):
        ...
        self.key = key
        self.keySlices = [ self.key[i*self.BLOCK_SIZE:(i+1)*self.BLOCK_SIZE] for i in range(self.ROUNDS) ]

    def __block_decrypt(self, block):
        block_int = int(block, 16)

        enc = int.from_bytes(block, "big")
        for i in range(self.ROUNDS):
            k = int.from_bytes(self.keySlices[i] , "big")
            enc &= k
            enc ^= k
    ...
    def decrypt(self, msg):
        ...
        for i in range(0, len(m), self.BLOCK_SIZE):
            e += self.__block_decrypt(m[i:i+self.BLOCK_SIZE])
        return e.encode()
