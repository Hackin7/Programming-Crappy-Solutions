...
function setUpTiling()
    ...
    for y = 1, mapHeight do
        ...
        for x = cameraScroll+1, cameraScroll+mapWidth do
            -- sky and bricks; this ID directly maps to whatever quad we want to render
            table.insert(tiles[y], cameraScroll+1, {
    ...
    for y = 1, mapHeight do
        for x = 1, mapWidth do
            ...
            and b.x >= (x - 1) * TILE_SIZE and b.x <= (x) * TILE_SIZE then
                b.floor = (y - 1) * TILE_SIZE - math.ceil(TILE_SIZE/2) -1
                havefloor = true
            end
...
function love.update(dt)
    ...
    elseif b.vx < 0 and b.x < cameraScroll + 100 and cameraScroll > 0 then
        cameraScroll = cameraScroll + b.vx * dt
    end
    love.keyboard.keysPressed = {}
end
